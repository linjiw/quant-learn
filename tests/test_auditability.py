...
def test_archive_research_outputs_preserves_prior_stance(tmp_path: Path, monkeypatch) -> None:
    ...
    with duckdb.connect(str(db_path)) as conn:
        conn.execute(
            ...
            INSERT INTO research_stance (
                stance_id, as_of_date, ticker, stance, stance_modifier, confidence,
            ...
            VALUES (
                'stance_test', '2026-05-01', 'AMD', 'constructive', 'factor_led',
                0.7, 'fixture', '', '', '', '', '', '', '', ?, ?
            )
            """,
            [now, now],
    ...
    assert archived.to_dict("records") == [
        {"run_id": "run_fixture", "stance_id": "stance_test", "ticker": "AMD"}
    ]
